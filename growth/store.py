...
Each profile is a separate "tenant": children and their measurements are
scoped to a profile so that two users of the same database (e.g. two
visitors to a Streamlit Cloud deployment) cannot see each other's data.

Profiles are identified by (name, passcode). Authentication lives in
`growth.auth`.
"""
...
DB_PATH = Path(__file__).resolve().parent.parent / "growth.db"
...
def _migrate_legacy_children(conn: sqlite3.Connection) -> None:
    ...
    # Create only the profiles table at this point. We can't run the full
    # SCHEMA yet because it includes an index on children(profile_id), and
    # the existing `children` table still has the pre-profile column set.
    ...
    # SQLite's ADD COLUMN can't create a NOT NULL column without a default,
    # so we add it nullable, backfill, and rely on application-level
    # invariants from here on (every insert goes through add_child).
...
def create_profile(name: str, passcode: str) -> Profile:
    """Create a new profile. Raises `auth.InvalidPasscode` if the passcode
    is too short, or `ValueError` if the name is taken / empty."""
    name = (name or "").strip()
    if not name:
...
def authenticate(name: str, passcode: str) -> Profile | None:
    """Return the profile if (name, passcode) match an existing record."""
    name = (name or "").strip()
...
def get_child(child_id: int, profile_id: int) -> Child | None:
    """Return the child only if it belongs to `profile_id` — otherwise None,
    so other profiles can't read each other's data even if they guess an id."""
    with connect() as conn:
        row = conn.execute(
            "SELECT id, profile_id, name, sex, dob FROM children "
...
# ---------- measurements (scoped via the child, which is scoped to a profile) ----------
...
def add_measurement(
    ...
    """Insert a measurement. Verifies child belongs to the profile first."""
...
def delete_measurement(profile_id: int, measurement_id: int) -> None:
    """Delete a measurement only if the owning child belongs to `profile_id`."""
    with connect() as conn:
        conn.execute(
